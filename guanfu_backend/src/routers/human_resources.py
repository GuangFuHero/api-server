...
def list_human_resources(
    ...
    resources = crud.get_multi(db, HumanResource, skip=offset, limit=limit, **filters)
    total = crud.count(db, HumanResource, **filters)
...
def create_human_resource(
    ...
    return crud.create_with_input(
        db, HumanResource, obj_in=resource_in, valid_pin=generate_pin()
    )
...
def get_human_resource(id: str, db: Session = Depends(get_db)):
    ...
    取得單一人力需求/角色
    """
    db_resource = crud.get_by_id(db, HumanResource, id)
