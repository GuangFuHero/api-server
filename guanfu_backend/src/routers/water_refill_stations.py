...
from ..models import WaterRefillStation
...
@router.get(
    "/",
    response_model=schemas.WaterRefillStationCollection,
    summary="取得飲用水補給站清單",
)
def list_water_refill_stations(
    ...
    stations = crud.get_multi(
        db, WaterRefillStation, skip=offset, limit=limit, **filters
    )
    total = crud.count(db, WaterRefillStation, **filters)
...
def create_water_refill_station(
    ...
    建立飲用水補給站
    """
    return crud.create(db, WaterRefillStation, obj_in=station_in)


@router.get(
    "/{id}", response_model=schemas.WaterRefillStation, summary="取得特定飲用水補給站"
)
def get_water_refill_station(id: str, db: Session = Depends(get_db)):
    """
    取得單一飲用水補給站
    """
    db_station = crud.get_by_id(db, WaterRefillStation, id)
...
@router.patch(
    "/{id}", response_model=schemas.WaterRefillStation, summary="更新特定飲用水補給站"
)
