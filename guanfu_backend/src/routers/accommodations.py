...
def list_accommodations(
    ...
    accommodations = crud.get_multi(
        db, Accommodation, skip=offset, limit=limit, **filters
    )
    total = crud.count(db, Accommodation, **filters)
    return {
...
def create_accommodation(
    ...
    建立住宿資源
    """
    return crud.create(db, Accommodation, obj_in=accommodation_in)
...
def get_accommodation(id: str, db: Session = Depends(get_db)):
    ...
    取得單一住宿資源
    """
    db_accommodation = crud.get_by_id(db, Accommodation, id)
...
@router.patch(
    "/{id}", response_model=schemas.Accommodation, summary="更新特定庇護所資料"
)
