...
@router.get(
    "/",
    response_model=schemas.SupplyItemCollection,
    summary="取得特定供應單物資項目清單",
)
def list_supply_items(
    ...
    items = crud.get_multi(db, SupplyItem, skip=offset, limit=limit, **filters)
    total = crud.count(db, SupplyItem, **filters)
...
def create_supply_item(
    ...
    parent_supply = crud.get_by_id(db, Supply, id=item_in.supply_id)
    if not parent_supply:
        raise HTTPException(
            status_code=400,
            detail=f"supplies with id {item_in.supply_id} does not exist.",
        )
    ...
    if item_in.received_count > item_in.total_number:
        raise HTTPException(
            status_code=400,
            detail="Received_count must be less than or equal to total_number.",
        )
    # remove unused columns
    supply_item = item_in.model_dump()
    del supply_item["valid_pin"]
    return crud.create(db, SupplyItem, obj_in=schemas.SupplyItemCreate(**supply_item))
...
def patch_supply_item(
    ...
    """
    # Check if parent supply_id exists
    db_supply_item = crud.get_by_id(db, SupplyItem, id=id)
    if not db_supply_item:
        raise HTTPException(status_code=404, detail="Supply Item not found.")
    if (
        db_supply_item.supply.valid_pin
        and db_supply_item.supply.valid_pin != item_in.valid_pin
    ):
        raise HTTPException(status_code=400, detail="The PIN you entered is incorrect.")
    # validate num
    if item_in.total_number is not None or item_in.received_count is not None:
        if db_supply_item.total_number == db_supply_item.received_count:
            raise HTTPException(
                status_code=400,
                detail="Received_count and total_number are locked because their values are equal; updates are not allowed.",
            )
        total_number = (
            item_in.total_number
            if item_in.total_number is not None
            else db_supply_item.total_number
        )
        received_count = (
            item_in.received_count
            if item_in.received_count is not None
            else db_supply_item.received_count
        )
...
def get_supply_item(id: str, db: Session = Depends(get_db)):
    ...
    取得單一物資項目
    """
    db_item = crud.get_by_id(db, SupplyItem, id)
    if db_item is None:
        raise HTTPException(status_code=404, detail="Supply Item not found")
    return db_item
