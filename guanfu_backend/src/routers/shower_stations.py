...
from ..enums import GeneralStatusEnum, ShowerFacilityTypeEnum
from ..models import ShowerStation
...
def list_shower_stations(
    ...
    stations = crud.get_multi(db, ShowerStation, skip=offset, limit=limit, **filters)
    total = crud.count(db, ShowerStation, **filters)
...
def create_shower_station(
    ...
    建立洗澡點
    """
    return crud.create(db, ShowerStation, obj_in=station_in)
...
def get_shower_station(id: str, db: Session = Depends(get_db)):
    ...
    取得單一洗澡點
    """
    db_station = crud.get_by_id(db, ShowerStation, id)
