...
def list_shelters(
    ...
    # shelters = select(Shelter).filter_by(**filters).offset(offset).limit(limit).all()
...
def create_shelter(shelter_in: schemas.ShelterCreate, db: Session = Depends(get_db)):
    ...
    建立庇護所
    """
    return crud.create(db, Shelter, obj_in=shelter_in)
...
def get_shelter(id: str, db: Session = Depends(get_db)):
    ...
    取得單一庇護所
    """
    db_shelter = crud.get_by_id(db, Shelter, id)
