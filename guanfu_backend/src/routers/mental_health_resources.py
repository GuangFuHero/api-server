...
def list_mental_health_resources(
    ...
    resources = crud.get_multi(
        db, MentalHealthResource, skip=offset, limit=limit, **filters
    )
    total = crud.count(db, MentalHealthResource, **filters)
...
def create_mental_health_resource(
    ...
    建立心理健康資源
    """
    return crud.create(db, MentalHealthResource, obj_in=resource_in)
...
def get_mental_health_resource(id: str, db: Session = Depends(get_db)):
    ...
    取得單一心理健康資源
    """
    db_resource = crud.get_by_id(db, MentalHealthResource, id)
...
@router.patch(
    "/{id}", response_model=schemas.MentalHealthResource, summary="更新特定心理健康資源"
)
