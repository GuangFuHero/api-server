...
from ..models import MedicalStation
...
def list_medical_stations(
    ...
    stations = crud.get_multi(db, MedicalStation, skip=offset, limit=limit, **filters)
    total = crud.count(db, MedicalStation, **filters)
...
def create_medical_station(
    ...
    建立醫療站
    """
    return crud.create(db, MedicalStation, obj_in=station_in)
...
def get_medical_station(id: str, db: Session = Depends(get_db)):
    ...
    取得單一醫療站
    """
    db_station = crud.get_by_id(db, MedicalStation, id)
