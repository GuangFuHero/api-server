...
def list_supplies(
    ...
    if embed == "all":
        query = query.options(joinedload(Supply.supplies))
...
@router.patch(
    "/{id}", response_model=schemas.Supply, status_code=200, summary="更新供應單"
)
def patch_supply(
...
def get_supply(id: str, db: Session = Depends(get_db)):
    ...
    db_supply = (
        db.query(Supply)
        .options(joinedload(Supply.supplies))
        .filter(Supply.id == id)
        .first()
    )
    if db_supply is None:
...
def distribute_supply_items(
    id: str,
    items_in: List[schemas.SupplyItemDistribution],
    db: Session = Depends(get_db),
):
    """
    ...
    updated_items = crud.distribute_items(
        db=db, supply_id=id, items_to_distribute=items_in
    )
    if not updated_items:
        raise HTTPException(
            status_code=400,
            detail="Invalid item IDs or distribution count exceeds total needed.",
        )
