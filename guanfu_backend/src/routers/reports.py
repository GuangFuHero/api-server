...
def list_reports(
    ...
    reports = crud.get_multi(db, Report, skip=offset, limit=limit, **filters)
    total = crud.count(db, Report, **filters)
    return {"member": reports, "totalItems": total, "limit": limit, "offset": offset}


@router.post(
    "/", response_model=schemas.Report, status_code=201, summary="建立回報事件"
)
def create_report(report_in: schemas.ReportCreate, db: Session = Depends(get_db)):
    """
    建立回報事件
    """
    return crud.create(db, Report, obj_in=report_in)
...
def get_report(id: str, db: Session = Depends(get_db)):
    ...
    取得單一回報事件
    """
    db_report = crud.get_by_id(db, Report, id)
